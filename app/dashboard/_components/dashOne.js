...
export default function DashOne ({setChartData}){
  ...
  Calculations(dummyData)
    ...
    return (
    <div className="w-full gap-2 flex flex-wrap lg:flex-nowrap items-center mb-8">

        {cards.map((c,id) => (
            <div key={id} className="lg:w-1/3 lg:h-40 w-full h-30 rounded-3xl flex justify-between bg-[#291043] px-4 py-4">
            <div className="flex flex-col items-start">
                <h2 className="md:text-lg font-bold">{c.head}</h2>
                <h1 className={`${c.color} text-base sm:text-2xl font-extrabold flex justify-center items-center mt-4 mb-2`}>{c.value}</h1>
                <h3 className="text-base sm:text-sm">{typeof c.stats === "string" ? <span>{c.stats}</span> : c.stats}</h3>
            </div>
            <div>
